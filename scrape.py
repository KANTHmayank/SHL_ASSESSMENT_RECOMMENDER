...
def parse_detail_page(url):
    ...
    duration = ""
    text = soup.find(
        text=re.compile(r"Approximate Completion Time.*?=\s*(\d+)", re.I)
    )
    if text:
    ...
    test_types = []
    tt_label = soup.find(text=re.compile(r"Test Type\s*:", re.I))
    if tt_label:
        container = tt_label.parent
        test_types = [
            span.get_text(strip=True)
            for span in container.find_all("span", class_="product-catalogue__key")
        ]
